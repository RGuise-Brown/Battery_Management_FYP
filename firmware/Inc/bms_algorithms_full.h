/*
 * bms_algorithms_full.h
 *
 *  Created on: Oct 13, 2025
 *      Author: rachaelguise-brown
 */

#ifndef BMS_ALGORITHMS_FULL_H_
#define BMS_ALGORITHMS_FULL_H_
...
// Battery capacity at different temperatures (YOUR INTERPOLATED DATA)
// TODO: Replace these placeholder values with your actual measured capacities
#define CAPACITY_VERY_COLD_UAH  8000000    // μAh at T < -20°C
#define CAPACITY_COLD_UAH       8800000    // μAh at -20°C to 0°C
#define CAPACITY_NORMAL_UAH     9800000    // μAh at 0°C to 20°C
#define CAPACITY_WARM_UAH       10500000   // μAh at T > 20°C

// Temperature thresholds (in decidegrees = 0.1°C)
#define TEMP_VERY_COLD_THRESH_DC  -200     // -20.0°C
#define TEMP_COLD_THRESH_DC       0        // 0.0°C
#define TEMP_NORMAL_THRESH_DC     200      // 20.0°C

// Transmission current
#define TX_CURRENT_MA          500         // 500mA pulse

// Voltage thresholds
#define VOLTAGE_CRITICAL_MV    2000        // 2.5V absolute minimum
#define VOLTAGE_WARNING_MV     2500        // 3.0V warning threshold

// SOC threshold (in per-mille)
#define SOC_LOW_THRESH_PM      300         // 30.0%

// Function 3 - Maximum allowed voltage drop during transmission
#define MAX_VOLTAGE_DROP_MV    500         // 500mV max drop
...
typedef struct {
    ...
    uint32_t accumulated_uah;      // Accumulated capacity in μAh (MATCHES BQ35100!)
 ...
 * 17.7% capacity variation with temperature.
 *
 ...
 * - T < -20°C: 8300 mAh capacity
 * - T > +5°C:  10500 mAh capacity
 * - Between:   Linear interpolation
 *
 * Example:
 *   state.temperature_dc = -223;  // -22.3°C
 *   state.accumulated_mah = 5400; // 5.4Ah used
 *   bms_update_soc_int(&state, &result);
 *   // result.adjusted_soc_pm = 349 (34.9% remaining)
 *   // result.available_capacity_mah = 8300
 */
 ...
 *
 * Analyzes temperature history to predict future capacity changes.
 * Warns if battery is getting colder (capacity will decrease) or warmer
 * (capacity will increase). Helps predict conditions before they impact SOC.
 *
 ...
 * - Determines impact on battery capacity
 *
 * Trend Statuses:
 * - COOLING_FAST:    > 2.0°C/hr drop → Capacity decreasing significantly
 * - COOLING:         > 0.5°C/hr drop → Capacity decreasing gradually
 * - STABLE:          ±0.5°C/hr      → No significant capacity change
 * - WARMING:         > 0.5°C/hr rise → Capacity increasing gradually
 * - WARMING_FAST:    > 2.0°C/hr rise → Capacity increasing significantly
 ...
 *          // Battery getting colder, capacity dropping
 *          // Consider delaying non-critical transmissions
 *      }
 *
 *      if (trend.status == TREND_COOLING_FAST) {
 *          // Rapid cooling - capacity loss imminent
 *          // Alert or take preventive action
 *      }
 *
 * Example Output:
 *   status = TREND_COOLING
 *   trend_dc_per_hour = -15 (-1.5°C/hr)
 *   predicted_temp_1hr_dc = 185 (18.5°C in 1 hour)
 *   capacity_will_decrease = true
 *   message = "Cooling (-1.5C). Monitor capacity."
 *
 * Practical Applications:
 * - Predict when to expect SOC changes due to temperature
 * - Delay transmissions if rapid cooling detected
 * - Schedule transmissions when warming (capacity improving)
