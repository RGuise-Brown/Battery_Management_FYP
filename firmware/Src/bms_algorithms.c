 ...
 *
 * Based on VERIFIED characterization data:
 * - Capacity at T < -20°C: 8.3 Ah (TESTED in freezer)
 * - Capacity at T > 20°C: 10.5 Ah (TESTED at room temp)
 * - High current transmission: 500mA pulses
 * - Critical voltage threshold: 2.5V under load
 *
 * NOTE: Only two temperature regions are used - values that can be
 * experimentally verified with available equipment.
 */
 ...
 *
 * TWO REGIONS ONLY (testable conditions):
 * - Cold (< 0°C): Use freezer-verified capacity
 * - Warm (≥ 0°C): Use room-temp-verified capacity
 *
...
void bms_update_soc_int(const BatteryState_Int_t *state, BmsResult_Int_t *result) {
    ...
    // Simple two-region approach based on what you can actually test
    if (temp_dc < TEMP_COLD_THRESH_DC) {
        // COLD REGION: Below 0°C (freezer-verified)
        // Use the capacity measured in your freezer tests
        available_uah = CAPACITY_VERY_COLD_UAH;  // 8.3 Ah = 8,300,000 μAh
    }
    else {
        // WARM REGION: 0°C and above (room-temp-verified)
        // Use the capacity measured at room temperature
        available_uah = CAPACITY_WARM_UAH;  // 10.5 Ah = 10,500,000 μAh
    }

    // Calculate remaining capacity
    // BQ35100 accumulated capacity is NEGATIVE for discharge
    // So we need to ADD it (since it's negative, this subtracts)
    int32_t used_uah = -state->accumulated_uah;  // Convert to positive "used"

    int32_t remaining_uah;
    if (used_uah < 0) {
        // Accumulated is positive (charging?) - treat as full battery
        remaining_uah = available_uah;
    } else if ((uint32_t)used_uah < available_uah) {
        remaining_uah = available_uah - (uint32_t)used_uah;
    } else {
    ...
    const char* temp_region = (temp_dc < TEMP_COLD_THRESH_DC) ? "COLD" : "WARM";

    snprintf(result->message, sizeof(result->message),
             "SOC: %u.%u%% | Available: %lu mAh @ %d.%u C (%s)",
             soc_pct, soc_dec, (unsigned long)available_mah, temp_whole, temp_dec, temp_region);
}
...
void bms_transmission_decision_int(const BatteryState_Int_t *state, BmsResult_Int_t *result) {
    ...
    // This is particularly important in your freezer tests
    if (state->temperature_dc < TEMP_COLD_THRESH_DC && result->adjusted_soc_pm < SOC_LOW_THRESH_PM) {
        ...
        snprintf(result->message, sizeof(result->message),
                 "BLOCK: Cold (%d C) + Low SOC (%u.%u%%) - High failure risk",
                 temp_whole, soc_pct, soc_dec);
        return;
    }

    // WARNING: Very cold temperature + Marginal voltage
    if (state->temperature_dc < TEMP_VERY_COLD_THRESH_DC && predicted_voltage_mv < VOLTAGE_WARNING_MV) {
        result->decision = DECISION_WARN;
        int16_t temp_whole = state->temperature_dc / 10;
        snprintf(result->message, sizeof(result->message),
                 "WARN: Marginal - Very Cold (%d C), Predicted V: %umV",
                 temp_whole, predicted_voltage_mv);
        return;
    }

    // WARNING: High impedance indicates battery degradation
    // Only check at warm temps where impedance should be lower
    if (state->impedance_mohm > IMPEDANCE_HIGH && state->temperature_dc >= TEMP_COLD_THRESH_DC) {
        result->decision = DECISION_WARN;
        snprintf(result->message, sizeof(result->message),
    ...
    if (result->adjusted_soc_pm < SOC_LOW_THRESH_PM) {  // < 30%
        result->decision = DECISION_WARN;
        uint16_t soc_pct = result->adjusted_soc_pm / 10;
        uint16_t soc_dec = result->adjusted_soc_pm % 10;
        snprintf(result->message, sizeof(result->message),
 ...
 *
 * @param voltage_before: Resting voltage before transmission (mV)
 * @param voltage_during: Voltage measured during transmission (mV)
 ...
 * Simplified for two-region model
 *
...
void bms_monitor_temperature_trend(TempTrendResult_t *result) {
    ...
    if (time_span_ms > 0) {
    	int64_t trend_calc = ((int64_t)trend_dc * 3600000LL) / time_span_ms;
    ...
    if (trend_dc < TEMP_TREND_COOLING_FAST) {
        ...
        if (result->capacity_will_decrease) {
            snprintf(result->message, sizeof(result->message),
                     "Cooling (%d.%u C). Approaching 0 C threshold.",
                     trend_whole, trend_dec);
        } else {
            snprintf(result->message, sizeof(result->message),
                     "Cooling (%d.%u C). No capacity change expected.",
                     trend_whole, trend_dec);
        }
    }
        ...
        result->capacity_will_decrease = false;
        // Check if we'll cross into warm region
        result->capacity_will_increase = (currently_cold && !will_be_cold);

        int16_t trend_whole = trend_dc / 10;
        int16_t trend_dec = trend_dc % 10;

        if (result->capacity_will_increase) {
            snprintf(result->message, sizeof(result->message),
                     "WARMING FAST (+%d.%u C). Will cross 0 C - Capacity increase!",
                     trend_whole, trend_dec);
        } else {
            snprintf(result->message, sizeof(result->message),
        ...
        if (result->capacity_will_increase) {
            snprintf(result->message, sizeof(result->message),
                     "Warming (+%d.%u C). Approaching 0 C threshold.",
                     trend_whole, trend_dec);
        } else {
            snprintf(result->message, sizeof(result->message),
                     "Warming (+%d.%u C). No capacity change expected.",
                     trend_whole, trend_dec);
        }
    }
    else {
...
void print_temp_trend_int(const TempTrendResult_t *result) {
    ...
    if (result->status != TREND_INSUFFICIENT_DATA) {
        ...
        UART_SendSignedNumber(result->trend_dc_per_hour / 10);
        UART_SendString(".");
        ...
        UART_SendSignedNumber(result->predicted_temp_1hr_dc / 10);
