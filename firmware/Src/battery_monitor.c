...
void BatteryMonitor_Init(void)
{
    ...
    // Initialize EOS mode gauge (I2C1)
    UART_SendString("\n[2/3] Initializing EOS mode (I2C1)...\r\n");
    BQ35100_I2C1_Init();

    // Initialize temperature sensor and ADC
    UART_SendString("\n[3/3] Initializing temperature sensor...\r\n");
...
void BatteryMonitor_ReadACC(BatteryMonitorData_t *data)
{
    ...
    data->acc_capacity_uah = BQ35100_I2C2_ReadAccumulatedCapacity();
...
void BatteryMonitor_ReadEOS(BatteryMonitorData_t *data)
{
    ...
    {
        data->eos_present = false;
        return;
    }

    data->eos_present = true;
    data->eos_voltage_mv = BQ35100_I2C1_ReadVoltage();
    data->eos_current_ma = BQ35100_I2C1_ReadCurrent();
    data->eos_temp_hundredths_c = BQ35100_I2C1_ReadTemperature_Int();
    data->eos_soh_percent = BQ35100_I2C1_ReadStateOfHealth();
    data->eos_scaled_r_mohm = BQ35100_I2C1_ReadScaledR();
    data->eos_measured_z_mohm = BQ35100_I2C1_ReadMeasuredZ();
    data->eos_pulse_count = BQ35100_I2C1_ReadEOSPulseCount();
    data->eos_short_trend = BQ35100_I2C1_ReadShortTrendAverage();
    data->eos_long_trend = BQ35100_I2C1_ReadLongTrendAverage();
    data->eos_flag_set = BQ35100_I2C1_IsEOSFlagSet();
}
...
void BatteryMonitor_ReadAll(BatteryMonitorData_t *data)
{
    ...
    // Read all subsystems
    BatteryMonitor_ReadACC(data);
    BatteryMonitor_ReadEOS(data);
    BatteryMonitor_ReadADC(data);
...
void BatteryMonitor_PrintSummary(const BatteryMonitorData_t *data)
{
    ...
    if (data->eos_present)
    {
        UART_SendString("+--- EOS MODE (I2C1) ----------------+\r\n");
        UART_SendString("| Voltage:    ");
        UART_SendNumber(data->eos_voltage_mv);
        UART_SendString(" mV\r\n");

        UART_SendString("| Impedance:  ");
        UART_SendNumber(data->eos_measured_z_mohm);
...
void BatteryMonitor_PrintDetailed(const BatteryMonitorData_t *data)
{
    ...
    if (data->eos_present)
    {
        UART_SendString("--- EOS MODE (I2C1) - Limited as Uncalibrated --------------------\r\n");
        UART_SendString("Voltage:             ");
        UART_SendNumber(data->eos_voltage_mv);
        UART_SendString(" mV\r\n");

        /*UART_SendString("Current:             ");
        if (data->eos_current_ma < 0)
        {
            UART_SendString("-");
            UART_SendNumber(-data->eos_current_ma);
        }
        else
        {
            UART_SendNumber(data->eos_current_ma);
        ...
        UART_SendNumber(data->eos_scaled_r_mohm);
        UART_SendString(" mOhm\r\n");*/
        ...
        UART_SendNumber(data->eos_soh_percent);
        UART_SendString(" %\r\n");

        UART_SendString("EOS Pulse Count:     ");
        UART_SendNumber(data->eos_pulse_count);
        UART_SendString(" / 120\r\n");

        UART_SendString("Short Trend Avg:     ");
        UART_SendNumber(data->eos_short_trend);
        UART_SendString("\r\n");

        UART_SendString("Long Trend Avg:      ");
        UART_SendNumber(data->eos_long_trend);
        UART_SendString("\r\n");

        UART_SendString("EOS Flag:            ");
        if (data->eos_flag_set)
        {
            UART_SendString("! SET - END OF SERVICE!\r\n");
        }
        else
        {
...
void BatteryMonitor_PrintTemperatures(const BatteryMonitorData_t *data)
{
    ...
    if (data->eos_present)
    {
        UART_SendString("EOS BQ35100 Temp:    ");
        int32_t whole = data->eos_temp_hundredths_c / 100;
        int32_t decimal = data->eos_temp_hundredths_c % 100;
        if (decimal < 0) decimal = -decimal;
        UART_SendNumber(whole);
        UART_SendString(".");
        if (decimal < 10) UART_SendString("0");
...
{
    BQ35100_I2C1_PerformEOSMeasurement();
}
